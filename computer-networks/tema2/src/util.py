...
import logging
logging.basicConfig(
    format=u'[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s', level=logging.INFO)
...
def compara_endianness(numar):
    ...
    '''
    print "Numarul: ", numar
    print "Network Order (Big Endian): ", [
          bin(ord(byte)) for byte in struct.pack('!H', numar)]
    print "Little Endian: ", [bin(ord(byte))
                              for byte in struct.pack('<H', numar)]
...
def construieste_mesaj_raw(ip_src, ip_dst, port_s, port_d, mesaj, protocol=socket.IPPROTO_UDP):
    ...
    for idx, octet in enumerate(ip_src_bytes):
        logging.debug("IP sursa byteul %i - %d in binar: %s ",
                      idx, ord(octet), bin(ord(octet)))

    logging.info("IP destinatie string: %s ", ip_src)
    for idx, octet in enumerate(ip_dst_bytes):
        logging.debug("IP sursa octetul %i - %d in binar: %s ",
                      idx, ord(octet), bin(ord(octet)))
    ...
    '''
    length = 8 + utf8len(mesaj)
    print "!!!!  LUNGIME MESAJ ", utf8len(mesaj)
    ...
    '''
        ...
        sumei de control
    '''
    mesaj_bytes = str.encode(mesaj)
    print "!!!!  MESAJUL IN BYTES ", mesaj_bytes
